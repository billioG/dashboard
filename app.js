...
// Configuración de Sedes (Latitud, Longitud)
const sedesConfig = {
...
window.addEventListener('load', () => {
    ...
    // CONEXIÓN DEL BOTÓN DE LOGIN (Solución al error anterior)
    const btnLogin = document.getElementById('btn-login'); // Asegúrate de agregar id="btn-login" en tu HTML si no funciona
    // O si usas el HTML anterior, la función onclick="loginSupabase()" funciona bien si este archivo carga.

    // Verificar si ya hay sesión iniciada
    const session = sb.auth.getSession().then(({ data: { session } }) => {
...
async function loginSupabase() {
    ...
    // CAMBIO: Usamos 'sb' en lugar de 'supabase'
...
async function checkUserRole(uid) {
    ...
    if(error || !profile) {
        ...
        // Si no hay perfil, intentamos cargar dashboard básico o mostrar error
        return;
    }
...
async function intentarGuardarConGPS() {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
            if(confirm("No pudimos obtener tu GPS. ¿Guardar de todos modos?")) {
...
function finalizarGuardado(sede, asistencia, notas, lat, lon, estadoGps, distancia) {
    // CAMBIO: sb.auth en lugar de supabase.auth
...
async function cargarDatosCoordinacion() {
    ...
    if(!tutores) {
        lista.innerHTML = '<p>No se encontraron tutores.</p>';
        return;
    }
...
async function syncData() {
    ...
    for (let item of offlineQueue) {
        item.tutor_id = user ? user.id : null;
